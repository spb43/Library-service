...
def getS3Path(s3Path,bucketName):
    #returned path from s3 url without last element and s3 link
    # print("******",s3Path)
    s3Path_rep=s3Path.replace(f"s3://{bucketName}/","")
    path_parts = s3Path_rep.split("/")
    l = len(path_parts)
    path_parts.remove(path_parts[l-1])
    path = os.path.join(*path_parts)
    # print("PATH____> ", path)
    return path

def getLastS3Object(s3Url):
    s3Path=s3Url.replace(f"s3://","")
    path_parts = s3Path.split("/")
    l = len(path_parts)
    return path_parts[l-1]

def downloadS3(bucket, s3Obj, dst_path):
    s3 = boto3.client('s3')
    #TODO Check folder
    print(f'Start download {s3Obj} to {dst_path}... ')
    s3.download_file(bucket, s3Obj, dst_path)

def uploadS3(uploadFile, bucket, s3Obj ):
    s3 = boto3.client('s3')
    s3.upload_file(uploadFile, bucket, s3Obj) 

def addExpIdColumn(csvPath,expId):
    df = pd.read_csv(csvPath)
    df['experimentid'] = expId
    df.to_csv(csvPath, index = False)

def sendMsgSqs(q, msg):
...
def run(filename):
    ...
    while True:
        ...
        if m is None:
            break
        message = m.to_dict()        
        messgsTypes.append(message['mavpackettype'])
        messages.append(message)
    messgsTypesSet = set(messgsTypes)
    id = uuid.uuid4()
    parsedDirectory = os.path.join(BASE_PATH, "parsed", str(id))
    print(parsedDirectory)
    if not os.path.exists(parsedDirectory):
        # If it doesn't exist, create it
        os.makedirs(parsedDirectory)
    for i in messgsTypesSet:
        msgs = []
        for j in messages:       
            if j['mavpackettype'] == i:
                msgs.append(j)
        df = pd.DataFrame(msgs)
        
        csvFileName = f"AP_{i}_{id}.csv"
        

        
        parsedFilePath = os.path.join(parsedDirectory, csvFileName)

        df.to_csv(parsedFilePath,index=False)
        columns = df.columns.values.tolist()
   
if __name__ == "__main__":
    arg = argparse.ArgumentParser(description='Ardupilot logs parser')
    arg.add_argument('input_arg', help='Enter path to file')
    file_path = arg.parse_args()
    print("Path to log file ---> ", file_path.input_arg)
    run(file_path.input_arg)